...
const initGame = (button, clickedLetter) => {
    ...
    if(wordUpper.includes(clickedLetter)) {
    ...
    if(!wordDisplay.innerHTML.includes('<li class="letter"></li>')) return gameOver(true);
}
...
document.addEventListener("keydown", (e) => {
    // Sirf A-Z letters ko accept karein
    if (e.key.match(/^[a-z]$/i)) {
        const letter = e.key.toUpperCase();
        // Keyboard buttons mein se us letter ko dhundhein aur click trigger karein
        const button = Array.from(keyboardDiv.querySelectorAll("button"))
